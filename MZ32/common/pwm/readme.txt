...
== blade & heartbeats ==
    ...
    ID of the blades & blade detection:
        (test_BLADE_WriteReadState);
        (test_HB_ConsecutiveBeacons_Calibrated);

== irda_pic == 

    Core of the IRDA functions:
        (test_IRDA_QueueToSend_Beacon_BT);
        (test_IRDA_QueueToSend_Beacon_BC);
        (test_IRDA_QueueToSend_BeaconAndMessage_MS);
        (test_IRDA_QueueToSend_BeaconAndMessage_MR);
...
== irda_ack == 
    ...
    Processes Dummy Pic's answer and reports it to the UART0:
