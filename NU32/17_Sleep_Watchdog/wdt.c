/*!\name      wdt.h
 *
 * \brief     Watchdog Timer every 4.096 seconds
 *            - Press USER button before 'j' to go to sleep; after to enter a
 *              faulty infinite loop. If sleep, the WDT will wake the PIC32.
 *            - If infinite loop, the WDT will reset the PIC32 and start over.
 * 
 * \author    Juan Gago
 *
 */
...
int main(void) {
  ...
  if(RCONbits.WDTO) {   // WDT caused the reset, but it was not from sleep mode
    RCONbits.WDTO = 0;  // Subsequent resets due to the reset button won't be interpreted as a WDT reset
  ...
  WDTCONbits.ON = 1;      // turn on the WDT (rollover of 4.096s in DEVCFG1)
                          // postcaler can be read from WDTCONbits.SWDTPS 
