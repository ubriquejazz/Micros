 ...
 *            Timing the Shadow Reg Set (SRS) versus 
 *            typical context save & restore (SOFT)
 *
 * \author    Juan Gago
 *
 */
...
int main(void) {
  ...
  while(1) 
  {
    ...
    while(IFS0bits.INT0IF);         // wait until the ISR clears the flag
    ...
    while(IFS0bits.INT1IF);         // wait until the ISR clears the flag
