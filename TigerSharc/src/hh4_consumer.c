...
volatile int rx_link2 = 0;
volatile int rx_link3 = 0;
...
void main(void)
{
	...
	// interrupt each time link 2 and 3 completes
    interrupt(SIG_LP2I, link2_handler); 
//    interrupt(SIG_LP3I, link3_handler); 
...
#pragma SIMD_for
			...
			{			    
			    ...
			    rfft2_re[LINE_SIZE/2 + i] = (float) 
			    	...
			    	(float) (ptr[LINE_SIZE/2 + i] & 0x0000FFFF);
			}
